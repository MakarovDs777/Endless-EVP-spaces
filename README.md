# Endless-EVP-spaces
Короче суть в том что нужно просто АБСОЛЮТИЗИРОВАТЬ случайность! Возвести случайность в АБСОЛЮТНУЮ АБСОЛЮТНОСТЬ! Только вопрос в том каким образом это сделать!? СЛУЧАЙНАЯ СЛУЧАЙНОСТЬ СЛУЧАЙНИСТВЕННО СЛУЧАЙНИСТВИНУЕТ, И СЛУЧАЙНИСТВЕННАЯ СЛУЧАЙНИСТВЕННЕНОСТЬ СТАНОВИТСЯ НАСТОЛЬКО СЛУЧАЙНИСТВЕННЕНО СЛУЧАЙНСТВИНЕННОЙ ЧТО ПЕРЕСТАЕТ БЫТЬ СЛУЧАЙНОСТЬЮ, И СТАНОВИТСЯ АБСОЛЮТНО НЕСЛУЧАЙНОЙ СЛУЧАЙНОСТЬЮ! СТАНОВЯСЬ АБСОЛЮТНЫМ СВОБОДНЫМ ПРОСТРАНСТВОМ ГДЕ МОЖНО СУЩЕСТВОВАТЬ БУДУЧИ ДАЖЕ НЕСУЩЕСТВУЮЩИЙ! Именно таким образом мы можем симулировать астрал! То есть иными словами возвести в абсолют случайность, и выразить это как такую функцию математический...

Я так прикинул, и получил такой примерный образ. Вот что у меня получилось:

[size=133]Волновая функция абсолютной случайности:[/size]

[math]ψ(x) = e^{i * θ(x)}[/math]
...
[size=133]Свойства этой волновой функции:[/size]
...
[size=133]Математическая интерпретация:[/size]
...
[size=200]Математическое представление[/size]
...
[size=133]Функция генерации значений[/size]
...
[size=133]Создание "квантовой пены"[/size]
...
[size=133]Четырехмерная интерпретация[/size]
...
[size=200]Зарисовка образа Абсолютно-Свободного-Пространства Астрала:[/size]
[code-color]import numpy as np
import random
...
while True:
    ...
    # Ждем шаг по времени[/code-color]
...
[size=133]Генерация волновых функций[/size]
...
[code-color]# Список волновых функций
wave_functions = []
...
[size=133]Обновление вероятности соединения[/size]

Вероятность соединения с волновой функцией i в точке (x, y, z) в момент времени t рассчитывается следующим образом:

[math]W(x, y, z, t) = \sum(w_i * exp(-d_i^2 | 2 * \sigma^2))[/math]

где:

[math]w_i[/math] - вес волновой функции i
[math]d_i[/math] - расстояние между точкой (x, y, z) и положением волновой функции i
[math]\sigma[/math] - параметр, определяющий ширину распределения вероятности

[size=133]Переход в другую волновую функцию[/size]
...
[code-color]if W(x, y, z, t) > threshold:
    ...
    z = max_wave_function["z"][/code-color]

[size=133]Обновление поля градиента[/size]
